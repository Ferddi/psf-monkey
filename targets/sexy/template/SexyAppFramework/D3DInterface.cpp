...
D3DTLVERTEX d3dtlVertices[MAX_VERTICES];

void D3DInterface::DrawPoints(const TriVertex theVertexList[], int theNumVertices, int theDrawMode)
{
	if (!PreDraw())
		return;

	SetupDrawMode(theDrawMode, 0xFFFFFFFF, NULL);

//	DWORD aColor = 0xFFFFFFFF;
//	float x = theRect.mX - 0.5f;
//	float y = theRect.mY - 0.5f;
//	float aWidth = theRect.mWidth;
//	float aHeight = theRect.mHeight;
	...
	{
		d3dtlVertices[i].sx = theVertexList[i].x;
		d3dtlVertices[i].sy = theVertexList[i].y;
		d3dtlVertices[i].sz = 0;
		d3dtlVertices[i].rhw = 1;
		d3dtlVertices[i].color = theVertexList[i].color;
		d3dtlVertices[i].specular = 0;
		d3dtlVertices[i].tu = 0;
		d3dtlVertices[i].tv = 0;
	}
	...
	if (!mTransformStack.empty())
	{
		...
		for (i=0; i<theNumVertices; i++)
		{
			p.x = theVertexList[i].x;
			p.y = theVertexList[i].y;
		
			p = mTransformStack.back() * p;
			p.x -= 0.5f;
			p.y -= 0.5f;
			d3dtlVertices[i].sx = p.x;
			d3dtlVertices[i].sy = p.y;				
		}
	}

	CheckDXError(mD3DDevice->SetTexture(0, NULL),"SetTexture NULL");
	CheckDXError(mD3DDevice->DrawPrimitive(D3DPT_POINTLIST, gVertexType, d3dtlVertices, theNumVertices, 0),"DrawPrimitive (Rect)");
}
...
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
void D3DInterface::SetClipRect(int theX, int theY, int theWidth, int theHeight)
{
	D3DVIEWPORT7 aD3DViewport;
	aD3DViewport.dwX = theX;
	aD3DViewport.dwY = theY;
	aD3DViewport.dwWidth = theWidth;
	aD3DViewport.dwHeight = theHeight;
	aD3DViewport.dvMinZ = 0; //-2048.0f;
	aD3DViewport.dvMaxZ = 1; //2048.0f;

	mD3DDevice->SetViewport(&aD3DViewport);
}
