...
ulong * DDInterface::PrimarySurfaceGetBits()
{
	DDImage* aDDImage = dynamic_cast<DDImage*>(mApp->GetWidgetManagerImage());
	if (aDDImage != NULL && mApp->GetWidgetManagerSurfaceLocked())
		aDDImage->UnlockSurface();

	ulong 					* mBits;
	DDSURFACEDESC			mLockedSurfaceDesc;
	LPDIRECTDRAWSURFACE 	aSurface = mDrawSurface;

	memset(&mLockedSurfaceDesc, 0, sizeof(mLockedSurfaceDesc));
	mLockedSurfaceDesc.dwSize = sizeof(mLockedSurfaceDesc);
	int aResult = aSurface->Lock(NULL, &mLockedSurfaceDesc, DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT, NULL);
	...
	int aRRound = (1 << (7 - mRedBits));
	int aGRound = (1 << (7 - mGreenBits));
	int aBRound = (1 << (7 - mBlueBits));
	...
	aSurface->Unlock(NULL);

	if (aDDImage != NULL)
		mApp->SetWidgetManagerSurfaceLocked(aDDImage->LockSurface());
