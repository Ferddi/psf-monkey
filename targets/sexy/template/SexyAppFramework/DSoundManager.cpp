...
SoundInstance* DSoundManager::GetChannel(unsigned int theChannel)
{
	if (theChannel > MAX_CHANNELS)
		return NULL;

	return mPlayingSounds[theChannel];
}

SoundInstance* DSoundManager::GetChannel(unsigned int theChannel, unsigned int theSfxID)
{
	if (theChannel > MAX_CHANNELS)
		return NULL;
	...
	// If the channel is being used by someone else, then force release it!
	if (mPlayingSounds[theChannel] != NULL)
	{
		mPlayingSounds[theChannel]->Release();
		delete mPlayingSounds[theChannel];
		mPlayingSounds[theChannel] = NULL;
	}

	if (mDirectSound==NULL)
	{
		mPlayingSounds[theChannel] = new DSoundInstance(this, NULL);
	}
	else
	{
		if (mSourceSounds[theSfxID] == NULL)
		...
		mPlayingSounds[theChannel] = new DSoundInstance(this, mSourceSounds[theSfxID]);
	}

	mPlayingSounds[theChannel]->SetBasePan(mBasePans[theSfxID]);
	mPlayingSounds[theChannel]->SetBaseVolume(mBaseVolumes[theSfxID]);
