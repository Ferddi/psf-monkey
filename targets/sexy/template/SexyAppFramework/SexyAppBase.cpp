...
void SexyAppBase::SwitchScreenMode(bool wantWindowed, bool is3d, bool force)
{
	...
	// Ferdi's comment: Removed because we may want to change the 
	// resolution of the game while staying at window or fullscreen.
...
void SexyAppBase::UpdateFTimeAcc()
{
	...
	{				
		...
		mUpdateFTimeAcc = min(mUpdateFTimeAcc + aDeltaTime, mMinUpdateFTimeAcc);
...
Sexy::DDImage* SexyAppBase::CreateImage()
{
	DDImage* anImage = new DDImage(mDDInterface);
	anImage->mFilePath = "\0";
//	anImage->SetBits(aLoadedImage->GetBits(), aLoadedImage->GetWidth(), aLoadedImage->GetHeight(), commitBits);	
...
MemoryImage * SexyAppBase::GetWidgetManagerImage()
{
	return mWidgetManager->mImage;
}

bool SexyAppBase::GetWidgetManagerSurfaceLocked()
{
	return mWidgetManager->mSurfaceLocked;
}

void SexyAppBase::SetWidgetManagerSurfaceLocked(bool flag)
{
	mWidgetManager->mSurfaceLocked = flag;
}
