...
Graphics::Graphics(Image* theDestImage)	
{
	...
	// Ferdi's comment: need to set this to true so images will use the color we set with SetColor.
	//mColorizeImages = false;
	mColorizeImages = true;
	mFastStretch = false;
...
void Graphics::DrawPoints(const TriVertex *theVertexList, int theNumVertices)
{
	mDestImage->DrawPoints(theVertexList, theNumVertices, mDrawMode);
}
...
{
	mDestImage->SetClipRect(theX, theY, theWidth, theHeight);
