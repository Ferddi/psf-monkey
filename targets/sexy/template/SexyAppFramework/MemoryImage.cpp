...
void MemoryImage::DrawPoints(const TriVertex theVertexList[], int theNumVertices, int theDrawMode)
{
	ulong* aBits = GetBits();
	...
	for (int aVertex = 0; aVertex < theNumVertices; aVertex++)
	{
		...
		ulong src = theVertex->color;
		int oldAlpha = src >> 24;
		
		if (oldAlpha == 0xFF)
		{
			aBits[((int)theVertex->y)*mWidth+((int)theVertex->x)] = src;
		}
		else
		{
			ulong* aDestPixels = &aBits[((int)theVertex->y)*mWidth+((int)theVertex->x)];
