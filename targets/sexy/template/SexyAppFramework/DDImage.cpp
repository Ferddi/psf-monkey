...
void DDImage::DrawPoints(const TriVertex theVertexList[], int theNumVertices, int theDrawMode)
{
	if (Check3D(this))
	{
		mDDInterface->mD3DInterface->DrawPoints(theVertexList,theNumVertices,theDrawMode);
		return;
	}
	...
	{
		MemoryImage::DrawPoints(theVertexList,theNumVertices,theDrawMode);
		return;
	}	
	...
	case Graphics::DRAWMODE_NORMAL:
		NormalDrawPoints(theVertexList,theNumVertices);
		break;
	case Graphics::DRAWMODE_ADDITIVE:
		AdditiveDrawPoints(theVertexList,theNumVertices);
		break;
	}
...
void DDImage::NormalDrawPoints(const TriVertex theVertexList[], int theNumVertices)
{
	if (mNoLock)
		return;
	...
	if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16)
	{
		for (int aVertex = 0; aVertex < theNumVertices; aVertex ++)
		{
			const TriVertex * theVertex = &theVertexList[aVertex];
			
			ulong theColor = theVertex->color;
			int alpha = theColor >> 24;
			int red = (theColor >> 16) & 0xFF;
			int green = (theColor >> 8) & 0xFF;
			int blue = theColor & 0xFF;
	
			ushort* aDestPixels = ((ushort*) mLockedSurfaceDesc.lpSurface) + ((int)theVertex->y * mLockedSurfaceDesc.lPitch/2) + (int)theVertex->x;

			if (alpha == 255)
			{
				ushort aColor = 
					((((red * aRMask) + aRRoundAdd) >> 8) & aRMask) |
					((((green * aGMask) + aGRoundAdd) >> 8) & aGMask) |
					((((blue * aBMask) + aBRoundAdd) >> 8) & aBMask);
			...
			{
				ushort src = 
					((((((red * alpha + 0x80) >> 8) * aRMask) + aRRoundAdd) >> 8) & aRMask) +
					((((((green * alpha + 0x80) >> 8) * aGMask) + aGRoundAdd) >> 8) & aGMask) +
					((((((blue * alpha + 0x80) >> 8) * aBMask) + aBRoundAdd) >> 8) & aBMask);
				int oma = 256 - alpha;
	...
	else if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32)
	{
		for (int aVertex = 0; aVertex < theNumVertices; aVertex ++)
		{
			...
			ulong* aDestPixels = ((ulong*) mLockedSurfaceDesc.lpSurface) + ((int)theVertex->y * mLockedSurfaceDesc.lPitch/4) + (int)theVertex->x;

			if (alpha == 255)
			{
				ulong aColor = 
					((((red * aRMask)) >> 8) & aRMask) |
					((((green * aGMask)) >> 8) & aGMask) |
					((((blue * aBMask)) >> 8) & aBMask);
			...
			{
				ulong src = 
					((((((red * alpha + 0x7F) >> 8) * aRMask) + aRRoundAdd) >> 8) & aRMask) +
					((((((green * alpha + 0x7F) >> 8) * aGMask) + aGRoundAdd) >> 8) & aGMask) +
					((((((blue * alpha + 0x7F) >> 8) * aBMask) + aBRoundAdd) >> 8) & aBMask);
				int oma = 256 - alpha;
...
void DDImage::AdditiveDrawPoints(const TriVertex theVertexList[], int theNumVertices)
{
	if (mNoLock)
		return;
	...
	if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 16)
	{
		for (int aVertex = 0; aVertex < theNumVertices; aVertex ++)
		{
			...
			ushort rc = ((red * alpha) / 255) >> (8-mDDInterface->mRedBits);
			ushort gc = ((green * alpha) / 255) >> (8-mDDInterface->mGreenBits);
			ushort bc = ((blue * alpha) / 255) >> (8-mDDInterface->mBlueBits);
	...
	else if (mLockedSurfaceDesc.ddpfPixelFormat.dwRGBBitCount == 32)
	{
		for (int aVertex = 0; aVertex < theNumVertices; aVertex ++)
		{
			...
			ulong rc = ((red * alpha) / 255) >> (8-mDDInterface->mRedBits);
			ulong gc = ((green * alpha) / 255) >> (8-mDDInterface->mGreenBits);
			ulong bc = ((blue * alpha) / 255) >> (8-mDDInterface->mBlueBits);
...
void DDImage::SetClipRect(int theX, int theY, int theWidth, int theHeight)
{
	if (Check3D(this))
	{
		mDDInterface->mD3DInterface->SetClipRect(theX, theY, theWidth, theHeight);
		return;
	}
}
